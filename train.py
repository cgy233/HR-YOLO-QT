...
sys.path.insert(0, osp.join(r'D:\Project\CHOPIN\HRNet', 'data'))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_dir', type=str, default='./data/hand_db',
...
def train(train_loader, model, criterion, optimizer):
    ...
    for i, items in enumerate(train_loader):
        ...
        target = items["target"].cuda()
        weight = items["weight"].cuda()
        kpt = items["kpt"]  # key points
        meta = items["meta"]
        normalize = items["normalize"]  # 标准化
        weight = weight.unsqueeze(-1)

        # compute output
        output = model(image)
        loss = criterion(output * weight, target * weight)
    ...
    _, avg_pck, _ = pose_pck_accuracy(preds.numpy(),
                                      kpts.numpy(),
                                      normalizes.numpy(),
                                      thr=0.2)
    return avg_pck
